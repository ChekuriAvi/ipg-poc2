    ...
    while (1) {
        ...
        for (; len < HdrSize; len += i) {
            if ((i = recv(sock, hdr + len, HdrSize - len, RSDScfg.RecvFlags)) <= 0) {
                if (!RSDScfg.TerminationRequested)
                    ROS_ERROR("RSDS_RecvHdr Error during recv (received: '%s' (%d))", hdr, len);
                return -1;
            }
        }
        if (hdr[0] == '*' && hdr[1] >= 'A' && hdr[1] <= 'Z') {
            ...
            if (RSDScfg.Verbose == 1 && nSkipped > 0)
                ROS_ERROR("RSDS: HDR resync, %d bytes skipped", nSkipped);
            return 0;
        }
...
static int RSDS_Connect(void)
{
    ...
    if ((he = gethostbyname(RSDScfg.MovieHost.c_str())) == NULL) {
        ROS_ERROR("RSDS: unknown host: %s", RSDScfg.MovieHost.c_str());
        return -2;
    }
    ...
    ROS_INFO("RSDS: Connected: %s", RSDScfg.sbuf + 1);
 ...
 */
static int RSDS_GetData(sensor_msgs::ImagePtr& image)
{
    unsigned int len = 0;
    ssize_t res = 0;
    ...
    if (sscanf(RSDScfg.sbuf, "*RSDS %d %s %f %dx%d %d", &Channel, ImgType, &SimTime, &ImgWidth, &ImgHeight, &ImgLen) == 6) {
        ...
        ROS_INFO("RSDS: not handled: %s", RSDScfg.sbuf);
    }
...
static void RSDS_PrintSimInfo()
{
    ...
    if (dtSimReal > 1.0) {
        ...
        ROS_INFO("Duration: %.3f (real) %.3f (sim) -> x%.2f", dtSimReal, RSDSIF.tLastSimTime, RSDSIF.tLastSimTime / dtSimReal);
        ROS_INFO("Channels: %d", RSDSIF.nChannels);
        ROS_INFO("Images:   %ld (%.3f FPS)", RSDSIF.nImagesSim, RSDSIF.nImagesSim / dtSimReal);
...
static void RSDS_PrintClosingInfo()
{
    ...
    if (dtSession > 1.0) {
        ...
        ROS_INFO("Images:   %ld (%.3f FPS)", RSDSIF.nImagesTotal, RSDSIF.nImagesTotal / dtSession);
...
int main(int argc, char **argv)
{
    ...
    if ((i = RSDS_Connect()) != 0) {
        ROS_ERROR("Can't initialise RSDS client (returns %d, %s)", i, i == -4 ? "No server": strerror(errno));
    }
